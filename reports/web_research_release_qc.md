...
Checks performed locally: `python -m unittest discover -s tests -q` (53 pass), `python scripts/validate_web_data.py` (pass), `python scripts/validate_brain_region_mapping.py` (pass), `npm test` (7 pass), `npm run build` (pass), and the desktop/mobile Playwright suite (14 pass, 8 intentional project-specific skips after test refactoring). The left/right/superior anatomical-orientation browser case passed. The production build still reports a large lazy Three.js chunk (~889 KB uncompressed) and the anatomical JSON is ~21 MiB uncompressed; hosts should serve compressed assets. No formal static-host deployment was performed.

CI follow-up: on commit `161bdbd`, the push browser run exceeded the 150-second timeout near the end of one monolithic desktop exploration test, while the pull-request browser run passed the same commit. The test now separates metric controls, regional evidence/disclosures, and Alzheimer comparison into fresh page contexts. The redundant final atlas screenshot test was removed because it could overwrite a correctly rendered image with an early blank canvas; the earlier anatomy QA test still captures that view. The local suite passed in the GitHub Actions test order; the refactor preserves every existing exploration assertion and adds a check for the default disease before switching.

The folded geometry is loaded asynchronously (~21 MiB). A separate local rerun reached the WebGL test while the page still displayed “Preparing WebGL brain…” and its default five-second canvas assertion expired. Browser checks now wait up to 30 seconds for the actual canvas before hit testing or capture; the WebGL hover/click case passed three consecutive repeated runs. The wait does not relax the region hit, data, or screenshot assertions.
