...
test('loads the real multi-disease atlas and supports metric controls', async ({ page }, testInfo) => {
  ...
  await expect(page.getByLabel('Spatial robustness legend')).toContainText('regions meet the project')
...
test('shows regional evidence, null summary, and downloads', async ({ page }, testInfo) => {
  ...
  await expect(page.getByLabel('Details for Putamen L')).toContainText('Technical details')
